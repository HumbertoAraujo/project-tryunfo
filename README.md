...
Neste projeto desenvolvemos um jogo no estilo Super Trunfo, onde será possível criar novas cartas e listar as cartas já existentes no baralho. Tudo isso de forma dinâmica, usando formulários e o estado da aplicação, onde entregamos requisitos que nos fizeram utilizar:
- Utilizar sintaxe JSX utilizada internamente no React;
- Explorar o método render() para renderizar os componentes criados através e arquitetura proposta (Padrão de pastas React);
- Utilizar import para "transitar" com os dados e componentes em diferentes arquivos e pastas;
- Criar componentes de classe em React;
- Fazer uso de props e de PropTypes para validação dos dados;
-Fazer uso de formulários em React;
-Para transmitir as informações internamente na aplicação, usamos callbacks e props;
-Criar e atualizar estados dos componentes,;
-Criamos funções para controlar os estados dos componentes por meio de eventos;
-Validação de todos os campos de acordo com a regra de negócio dos requisitos para os fomulários e renderizações.

!!!!!IMPORTANTE este foi o projoeto entregue conforme requisitos do BOT avaliador, vou refatorar e realizar a estilização!!!!!
